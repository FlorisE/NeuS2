...
public:
	...
	void reload_network_from_file(const std::string& network_config_path, const std::string& network_output_path="output");
	...
	void save_snapshot_incremental(const std::string& filepath_string, bool include_optimizer_state);
	...
	std::chrono::time_point<std::chrono::steady_clock> m_training_one_time_frame_point;
	...
	struct TrainableGlobalMove {
		...
		// std::shared_ptr<tcnn::DifferentiableObject<float, precision_t, float>> delta_network;
