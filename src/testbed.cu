...
__global__ void add_mesh_pos(
	const uint32_t n_elements,
	const float* mesh_vex,
	const uint32_t dim,
...
bool Testbed::training_network_next_frame() {
	...
	if (current_training_time_frame >= 1 && m_incremental_reinit_sdf_mlp && current_training_time_frame % m_incremental_reinit_sdf_mlp_iters == 0){
		tcnn::pcg32 sdf_rng{1337};
		m_nerf_network->initialize_sdf_mlp_params(sdf_rng, m_trainer->params_full_precision(), m_trainer->params(), m_trainer->params_inference(), m_trainer->params_backward(), m_trainer->param_gradients());
	}
...
void Testbed::reset_network() {
	...
	if (to_lower(encoding_config.value("otype", "OneBlob")).find("grid") != std::string::npos) {
		...
		if (m_testbed_mode == ETestbedMode::Nerf && m_per_level_scale <= 0.0f && m_num_levels > 1) {
		...
		m_top_grid_resolution = (uint32_t)ceilf(exp2f((m_num_levels - 1) * std::log2(m_per_level_scale)) * m_base_grid_resolution - 1.0f) + 1;
	...
	{
		json& globalmove_loss_config = config.contains("globalmove") && config["globalmove"].contains("loss") ? config["globalmove"]["loss"] : loss_config;
		json& globalmove_optimizer_config =  config.contains("globalmove") && config["globalmove"].contains("optimizer") ? config["globalmove"]["optimizer"] : optimizer_config;

		m_global_move.loss_type = string_to_loss_type(globalmove_loss_config.value("otype", "L2"));
		m_global_move.delta_network = m_nerf_network->delta_network();
		m_global_move.optimizer.reset(create_optimizer<precision_t>(globalmove_optimizer_config));
		m_global_move.trainer = std::make_shared<Trainer<float, precision_t, float>>(m_global_move.delta_network, m_global_move.optimizer, std::shared_ptr<Loss<float>>{create_loss<float>(globalmove_loss_config)}, m_seed);
...
void Testbed::reset_network_incremental() {
	...
	(*leaf_optimizer_config)["learning_rate"] = (*leaf_optimizer_config)["after_learning_rate"];
...
void Testbed::save_transform(const std::string& filepath_string) {
	...
	save_accumulated_rotation.resize(sizeof(precision_t) * m_nerf_network->rotation()->n_params());
	save_accumulated_transition.resize(sizeof(precision_t) * m_nerf_network->transition()->n_params());

	tcnn::linear_kernel(save_global_movement_rotation_6d_kernel<precision_t>, 0, m_training_stream, 1u,
			m_nerf_network->delta_network()->rotation()->params(), m_nerf_network->delta_network()->transition()->params(),
			m_nerf_network->rotation()->params(), m_nerf_network->transition()->params(),
	...
	CUDA_CHECK_THROW(cudaMemcpy(rotation_quat.data(), save_accumulated_rotation.data(), (9) * sizeof(precision_t), cudaMemcpyDeviceToHost));
	CUDA_CHECK_THROW(cudaMemcpy(transition.data(), save_accumulated_transition.data(), (3) * sizeof(precision_t), cudaMemcpyDeviceToHost));
...
void Testbed::save_snapshot_incremental(const std::string& filepath_string, bool include_optimizer_state) {
	fs::path filepath = filepath_string;
	m_network_config["snapshot"] = m_trainer->serialize(include_optimizer_state);
