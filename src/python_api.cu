...
PYBIND11_MODULE(pyngp, m) {
	...
	testbed
		...
		.def("reload_network_from_file", &Testbed::reload_network_from_file, py::arg("path")="", py::arg("output_path")="", "Reload the network from a config file.")
		...
		.def("save_transform", &Testbed::save_transform, py::arg("path"), "Save a snapshot of the predicted transformation of currently trained model")
	...
	testbed
		...
		.def_readonly("first_frame_max_training_step", &Testbed::first_frame_max_training_step)
		.def_readonly("next_frame_max_training_step", &Testbed::next_frame_max_training_step)
		.def_readonly("all_training_time_frame", &Testbed::all_training_time_frame)
